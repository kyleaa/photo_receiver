defmodule PhotoReceiver.CameraChannel do
  ...
  def handle_in("print", %{"filename"=>filename, "printer"=>printer}, socket) do
    ...
    Logger.debug "lp -d #{printer} -o landscape -o fit-to-page -o media=Postcard.Fullbleed files/#{filename}"
    System.cmd "lp", ["-d", "#{printer}", "-o", "landscape", "-o", "fit-to-page","-o","media=Postcard.Fullbleed", "files/#{filename}"]
    {:noreply, socket}
  end
